...
double elapsed_time = 0.0;   // Elapsed time for FPS calculation.
...
glm::vec3 ui_cube_position;  // UI cube position.
glm::vec3 ui_cube_scale;     // UI cube scale.
bool is_mouse_hovering_ui = false; // Flag for mouse hovering over the UI cube.
bool is_mouse_clicked = false;     // Flag for mouse click.
bool is_texture_changed = false;   // Flag for texture change on click.
size_t active_texture_index = 0;   // Index of the active texture.
// Time variables.
...
void update()
{
	...
	// UI cube bounds.
	const float ui_cube_left = ui_cube_position.x - ui_cube_scale.x / 2.0f;
	const float ui_cube_right = ui_cube_position.x + ui_cube_scale.x / 2.0f;
	const float ui_cube_top = ui_cube_position.y - ui_cube_scale.y / 2.0f;
	const float ui_cube_bottom = ui_cube_position.y + ui_cube_scale.y / 2.0f;
...
void render()
{
	...
	glm::mat4 orthographic_projection = glm::ortho(0.0f, static_cast<float>(camera.get_window_width()),
		0.0f, static_cast<float>(camera.get_window_height()), -1.0f, 1.0f);
